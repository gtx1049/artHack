...
void memcpy1(unsigned char *d, unsigned char *s, int l)
{
	int i;
...
void encryptAES(const u1byte in_blk[16], u1byte out_blk[16])
{   u4byte  b0[4], b1[4], *kp;
...
void decryptAES(const u1byte in_blk[16], u1byte out_blk[16])
{   u4byte  b0[4], b1[4], *kp;
    ...
    )
{
    ...
    if(encrypt)
    {
        ...
        for(i = 0; i < nr_blocks; i++)
        {
			...
			encryptAES(feedback, feedback);
            /* write cipher to output */
            if(out_data)
                memcpy1(&out_data[i * 16], feedback, 16);
        }
        if(out_cbc_mac)
            memcpy1(out_cbc_mac, feedback, 16);
    }
    else
    {
		int i;
        ...
        for(i = 0; i < nr_blocks; i++)
        {
            ...
            memcpy1(tmpin, &in_data[i * 16], 16);
            
			//DecryptAES(&in_data[i * 16], key, &out_data[i * 16]);
			
			decryptAES(&in_data[i * 16], &out_data[i * 16]);

            /* xor it with iv */
            xor_(&out_data[i * 16], feedback, 16);
            /* copy cipher to iv */
            memcpy1(feedback, tmpin, 16);
        }
    }
}
...
int init_aes(unsigned char *szpass)
{
	int i;
	int length;
	unsigned char *p = szpass;
	...
	if (length < 32)
	{
		memcpy1(password, szpass, length);
	}
	else if (length >= 32)
	{
		memcpy1(password, szpass, 32);
	}

	set_key((unsigned char *)password, 256);
...
void cryptdata(unsigned char *buffer, int lba, int nsectors)
{
	int i;
	byte iv[16] = {0};
	for (i = 0; i < nsectors; i++)
	{
		//RC5_CBC_Encrypt_Update(lba, (RC5_WORD *)buffer, (RC5_WORD *)buffer);
		*(int *)&iv[0] = lba;
		cbc_mac(buffer, buffer, SECTOR_SIZE / 16, (unsigned char *)password, iv, NULL, 1); // crypt:   p -> q
		buffer += 512;
		lba++;
	}
}

void decryptdata(unsigned char *buffer, int lba, int nsectors)
{
	int i;
	byte iv[16] = {0};
	for (i = 0; i < nsectors; i++)
	{
		//RC5_CBC_Decrypt_Update(lba, (RC5_WORD *)buffer, (RC5_WORD *)buffer);
		*(int *)&iv[0] = lba;
		cbc_mac(buffer, buffer, SECTOR_SIZE / 16, (unsigned char *)password, iv, NULL, 0); // crypt:   p -> q
		buffer += 512;
		lba++;
	}
}
...
// encrypt=1, crypt the data
void cryptsector(unsigned char *buffer, unsigned int lba, int nsectors, unsigned int key, int encrypt)
{
	if (encrypt)
	{
		cryptdata(buffer, lba, nsectors);
	}
	else
	{
		decryptdata(buffer, lba, nsectors);
	}
}
...
// #include "time.h"
// 
// byte sysdata[64*1024];
// byte cipher[64*1024];
// char szStr[100];
// 
// #define SUFFIX "111122223333"
// 
// /*
// int isfirstkey(char *str, int len)
// {
// 	// "5678"
// 	return (            str[0] == '5'  &&
// 		    (len < 2 || str[1] == '6') &&
...
// 
// }
// 	char *key = "aaaa"SUFFIX; // "aaaa" is changable; "111122223333" is fixed.
// 
// -- 5678aaaa will open the secure disk....
// 
// */
// int main(void)
// {
...
// 
//     byte iv[16] = {0};
// 	char *key = "bbbb"SUFFIX; // "bbbb" is changable; "111122223333" is fixed.
// 
// 	if (1)
// 	{
// 		int len = 1*1024*1024;
// 		byte *p = (byte *)malloc(len);
// 		byte *q = (byte *)malloc(len);
// 		byte *r = (byte *)malloc(len);
// 
// 		time_t t1 = time(NULL);
// 		time_t t2;
// 		int k=0;
...
// 		do {
// 			memset(p, 0x3, len);
// 			set_key((unsigned char *)key, 128);
// 
// 			cbc_mac(p, q, len/16, (unsigned char *)key, iv, NULL, 1); // crypt:   p -> q
// 			cbc_mac(q, r, len/16, (unsigned char *)key, iv, NULL, 0); // decrypt: q -> r
// 			printf(".");
// 			k++;
// 			t2 = time(NULL);
// 		} while ((t2-t1)<5);
// 
// 
// 		printf("\nAES crypted speed = %d MB/s", (k*2)/(t2-t1));
// 
// 		//return 0;
// 
// 	}
...
//  cbc_mac(plain, cipher, 512/16, key, iv, NULL, 1);
//  cbc_mac(cipher, plain2, 512/16, key, iv, NULL, 0);
//
//  Pretty(cipher,32,"crypted:      ");
//  Pretty(plain2,32,"decrypted:    ");

  memset(iv, 3, sizeof(iv));
  cbc_mac(plain, cipher, 512/16, key, iv, NULL, 1);
  cbc_mac(cipher, plain2, 512/16, key, iv, NULL, 0);
  ...
  memset(iv, 3, sizeof(iv));
  cbc_mac(plain, plain, 512/16, key, iv, NULL, 1);
  cbc_mac(plain2, cipher, 512/16, key, iv, NULL, 1);
  Pretty(plain,32,"crypted:      ");
  Pretty(cipher,32,"crypted:      ");
  cbc_mac(plain, plain, 512/16, key, iv, NULL, 0);
  cbc_mac(cipher, plain2, 512/16, key, iv, NULL, 0);

  Pretty(plain,32,"decrypted:    ");
  Pretty(plain2,32,"decrypted:    ");
